...
function attachLatencyListeners() {
  ...
  document.querySelectorAll('input[type="radio"]').forEach(function(radio){
    const new_radio = radio.cloneNode(true);
    radio.parentNode.replaceChild(new_radio, radio);

    var name = new_radio.getAttribute('name');
    if (!name) return;
    var questionId = name;

    // Start time stamp
    if (!window.latencyTimers) window.latencyTimers = {};
    new_radio.addEventListener('mousedown', function() {
    ...
    new_radio.addEventListener('click', function() {
      ...
      if (window.Shiny) {
        window.Shiny.setInputValue('latency_event', {
          ...
          responseValue: new_radio.value,
          latency: latency
        }, {priority: 'event'});
      }
      // For debugging:
      console.log("📊 Recording latency:", {questionId, responseValue: new_radio.value, latency});
    });
  });
}

// Attach on load and every time DOM changes (new question/page)
